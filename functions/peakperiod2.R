#peakperiod2.R for returning list of results instead of opdf.
peakperiod2 <- function(df, peakstart, peakend, comparedemand=FALSE, comparegeneration=FALSE){
  ...
  peakcoin <- sprintf("%.2f", round(peakcoin,2))
  
  
  #maximum netKW of all the data and opdf
  maxnet <- round(max(df$netKW), 2)
...
#   }
  ...
  loadfactor <- sprintf("%.2f", round(loadfactor,2))
...
#   }
  ...
  results <- list(maxnet, maxtime, peakcoin, loadfactor)
